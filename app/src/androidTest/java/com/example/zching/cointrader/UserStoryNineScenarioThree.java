...
Then I can read about the mining process for that coin.
 */

@LargeTest
@RunWith(AndroidJUnit4.class)
public class UserStoryNineScenarioThree {

    @Rule
    public ActivityTestRule<HomeScreen> mActivityTestRule = new ActivityTestRule<>(HomeScreen.class);

    @Test
    public void userStoryNineScenarioThree() {
        ...
        ViewInteraction textView = onView(
                allOf(withId(R.id.textView34), withText("How to Obtain:"),
                        childAtPosition(
                                childAtPosition(
                                        withId(android.R.id.content),
                                        0),
                                4),
                        isDisplayed()));
        textView.check(matches(withText("How to Obtain:")));
